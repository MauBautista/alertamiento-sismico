...
async def _features_calibrated(client) -> bool:
    r = await client.get(f"/telemetry/sites/{S_A}/features", headers=_auth())
    assert r.status_code == 200, r.text
    return r.json()["calibrated"]


async def test_uncalibrated_by_default(client, seed) -> None:
    ...
    """
    assert await _features_calibrated(client) is False


async def test_declaring_the_source_calibrates_the_site(client, seed) -> None:
    await _set_calibration(SENSOR_A, "stationxml:AM.R4F74.2026-07-09")
    assert await _features_calibrated(client) is True


async def test_one_uncalibrated_sensor_uncalibrates_the_whole_site(client, seed) -> None:
...
async def test_retired_sensors_do_not_block_calibration(client, seed) -> None:
...
async def test_metrics_carry_the_same_flag(client, seed) -> None:
    """El histórico (caggs) hereda el mismo `max_pga_g` sin calibrar: mismo aviso."""
    r = await client.get(f"/telemetry/sites/{S_A}/metrics", headers=_auth())
    assert r.status_code == 200, r.text
    assert r.json()["calibrated"] is False

    await _set_calibration(SENSOR_A, "stationxml:AM.R4F74")
    r2 = await client.get(f"/telemetry/sites/{S_A}/metrics", headers=_auth())
    assert r2.json()["calibrated"] is True


async def test_site_without_visible_sensors_is_not_calibrated(client, seed) -> None:
