    ...
    Requisito(
        ...
        afirmaciones=(
            ...
            Afirmacion(
                ...
                (),
                hueco=(
                    "**El hueco que queda es el despliegue, y solo ése.** ⚠️ Esta nota decía "
                    "hasta el 2026-09-22 que la perilla «sigue en `False`» y que era **la única "
                    "brecha multi-tenant viva**, y las dos cosas dejaron de ser verdad el "
                    "2026-09-12: `T-7.06` la encendió en el despliegue "
                    "(`deploy/cloud/deploy.sh · TAKAB_API_CONSOLE_SCOPE_ENFORCED=true`) y el "
                    "censo de conformidad la mide `true` **dentro de la instancia**. Desde el "
                    "2026-09-22 el valor está ratificado en "
                    "`settings.VALOR_RATIFICADO_EN_PRODUCCION` y un test se pone rojo si alguien "
                    "lo cambia en `deploy.sh`.\n\n"
                    "  Esta fila **sigue sin subir a `CUBIERTO`**, y no es contradicción: lo que "
                    "afirma es que el servidor DESPLEGADO filtra entre clientes reales, y eso se "
                    "acredita con dos tenants y dos sesiones —el criterio 3 de `T-2.89`—, no con "
                    "la suite. Lo que cambió es que el hueco pasó de **construcción** a "
                    "**acreditación**.\n\n"
                    "  > ### ✅ Lo que SÍ cambió el 2026-08-22 "
                    "([`D-18`](DECISIONES-MAURICIO.md#d-18))\n"
                    "  > Hasta esa fecha esta nota decía que **ningún test recorría el HTTP con "
                    "el filtro puesto**, y ya no es cierto: los tres tests de "
                    "`api/tests/api/test_console_scope.py` están **parametrizados por la "
                    "perilla** y fijan la conducta a los dos lados —con el filtro puesto, un "
                    "claim vacío no ve ningún sitio, `/me` lo declara y el `scope_gap` deja de "
                    "escribirse—.\n"
                    "  >\n"
                    "  > **Y decía algo peor, que era un error de conteo con precio.** Afirmaba "
                    "que **dos** tests HTTP fijaban la conducta no impuesta y que encender la "
                    "perilla pondría la suite en rojo. **Eran TRES**: faltaba "
                    "`::test_fase_A_el_hueco_queda_auditado_una_sola_vez`, que con la perilla "
                    "encendida no ve ninguna fila `scope_gap` porque `auth/scope.py` devuelve "
                    "`gap=False`. Un aviso que enumeraba a mano enumeró de menos, y el que "
                    "faltaba habría saltado **en mitad de la ventana AWS** — exactamente lo que "
                    "`D-18` mandó evitar. **Hoy no hay rojo que esperar por ninguno de los "
                    "tres:** la perilla se enciende con la suite verde antes y después."
                ),
            ),
        ),
    ),
