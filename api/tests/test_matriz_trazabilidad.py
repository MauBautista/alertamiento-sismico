    ...
    Requisito(
        ...
        afirmaciones=(
            ...
            Afirmacion(
                ...
                (),
                hueco=(
                    "**El hueco que queda es el despliegue, y solo ése.** En producción la "
                    "perilla sigue en `False` (`api/src/takab_api/settings.py`): el servidor "
                    "desplegado NO filtra, y es la única brecha multi-tenant viva — la que "
                    "`T-2.89` existe para cerrar, con su secuencia obligada (recorrer los "
                    "`scope_gap` → asignar alcance → **encender al final**). Esta fila no sube "
                    "a `CUBIERTO` por tests: lo que afirma es una conducta *del despliegue*, y "
                    "eso lo acredita el `apply`, no la suite.\n\n"
