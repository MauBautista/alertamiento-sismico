// Cliente WS live de la consola (T-1.27 · gate #5: WS nativo).
//
// Envuelve el canal /ws de T-1.22 con la semántica que necesita la UI:
// auth-first-frame → ready → subscribe(topic, cb); reconexión con backoff
// exponencial + jitter (1 s..30 s) y RE-subscribe al reabrir; close 4401 ⇒
// onUnauthorized (sesión inválida — la app decide mandar a login); staleness
// por topic vía lastFrameAt. Los shapes de los frames vienen del SDK
// (@takab/sdk ws.ts, generados de ws/protocol.py) — aquí NO se inventan.
...
export interface LiveSocketOptions {
  /** URL absoluta ws(s)://…/ws (ver `liveWsUrl`). */
  url: string;
...
export class LiveSocket {
  ...
  private scheduleReconnect(): void {
    ...
    const delay = Math.min(
      this.options.backoffBaseMs * 2 ** this.attempt,
