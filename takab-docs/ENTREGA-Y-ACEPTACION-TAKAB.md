...
| **Alcance por sitio dentro de un mismo cliente** | **✅ IMPUESTO en producción desde el 2026-09-12** (`T-7.06`, decisión [`D-18`](DECISIONES-MAURICIO.md#d-18)). El aislamiento **entre clientes** lo impone la base de datos (§3.4) y, desde esa fecha, un operador ve **solo sus sitios**: la perilla `console_scope_enforced` está encendida en el despliegue y el censo de conformidad la mide encendida **dentro de la instancia**. El valor está ratificado —`settings.VALOR_RATIFICADO_EN_PRODUCCION`— y un test se pone rojo si alguien lo cambia. **Ya no hay brecha multi-tenant viva.** ⚠️ Esta fila afirmó lo contrario hasta el 2026-09-22: subestimaba el producto ante el cliente durante diez días | Lo que queda de `T-2.89` es **acreditación, no construcción**: la prueba entre dos clientes contra el entorno desplegado, que exige una sesión con MFA |
