...
| **E5** | 🟢 | `dictamen/layout.py:103-108`; `routers/reports.py:97-111,115-130` | `test_pdf.py::test_el_mismo_modelo_produce_los_mismos_bytes`; `test_que_identifica_la_huella.py::test_NINGUN_campo_del_modelo_es_CIEGO_a_la_huella`; `test_reports.py::test_report_generates_pdf_evidence_and_audits`; `test_append_only.py::test_delete_blocked[evidence_objects]` | El PDF se hashea al generarse, se registra en tabla que no admite borrado y se audita dos veces; el determinismo del sello está defendido y el circuito se ejerció con hash idéntico verificado. **⚠️ Corregido en `T-7.43`:** esta fila citaba `::test_la_huella_de_contenido_es_estable_y_cambia_con_el_contenido`, que pasa en verde con el reloj clavado por su fixture y por tanto no defendía lo que se le atribuía. El censo derivado que sí lo defiende —ningún campo ciego, 58 de 58— lo sustituye. Y el 🟢 no significa que dos exportaciones coincidan: **no coinciden nunca**, por diseño. |
