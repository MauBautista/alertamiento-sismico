...
- `[x]` **GATE-HW G-01:** reinicio en frío en cada Pi con verificación del backend activo — **acreditado el 2026-08-24 en gw-dev-0001, con el layout A/B** (ver la tabla). Queda pendiente en los demás gabinetes cuando existan.
...
|---|---|---|---|---|---|---|
| G-01 | Restart en frío del Pi (L1) | `sudo reboot` en cada Pi con gabinete armado | `takab-edge` y `takab-gpio` activos; backend gpiozero = lgpio verificado en el arranque (journal); relés responden; sin caída a sysfs **ACREDITADO (2026-08-24)** · gw-dev-0001, **con layout A/B**. **1-2** `active`/`active`, `NRestarts` **0 y 0** · **3** `LGPIOFactory (lgpio)`, sin caída a sysfs · **4** cerrojo `pid=740`, `unit=takab-gpio`, `flock=9` · arrancó **desde el symlink** (`releases/20260824T034254Z-67de47c/edge`) · nube `online` (RTT 75 ms), SeedLink reconectado, `journalctl -p err` **vacío** · **5** prueba de actuadores 20:34:07 → `ok: true`, **5 canales con `readback_ok: true`**; journal del dueño: «prueba local de actuación: OK (5 canales — NO es alerta real)». Sirena y estrobo **vistos y oídos** por Mauricio, y el gabinete volvió a reposo solo (`siren_sounding: false`, ambos relés desenergizados) al vencer el sostén. **Alcance honesto:** en este sitio el perfil de equipamiento declara SOLO `siren` y `strobe`; el `readback_ok` de `gas_valve`/`elevator`/`door_retainer` prueba **el camino del pin**, no que se haya movido una válvula. | **OK** | 2026-08-24 |
