...
| #5 API en vivo | `[RATIFICADO 2026-07-06 · T-1.22]` **REST + WebSocket nativo, SIN GraphQL.** Implementado y verificado E2E vivo (commit→frame 214 ms en **una observación**, no un percentil — [`MEDICIONES-TAKAB.md`](MEDICIONES-TAKAB.md); fan-out LISTEN/NOTIFY con RLS como autoridad de tenancy) — `TASKS.md` T-1.22 | Gate cerrado. GraphQL subscriptions queda pos-MVP como **T-3.15**, solo si un cliente lo pide |
