...
| RO-10.c | La salud del dispositivo llega por latido periódico y etiquetado como tal. | `CUBIERTO` | `edge/tests/test_health.py:249`<br>`test_heartbeat_thread_emits_periodic_snapshots`<br>`api/tests/test_ingest_handlers.py:441`<br>`test_health_default_reason_is_heartbeat` | Con `heartbeat_s=0.05` salen ≥3 instantáneas por temporizador.<br>En la nube, una salud sin motivo se persiste como `heartbeat`. |
